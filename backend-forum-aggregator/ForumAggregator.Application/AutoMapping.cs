...
public class AutoMapping : Profile
{
    public AutoMapping()
    {
        ...
        CreateMap<ForumAggregator.Domain.CommentRegistry.Comment, ForumAggregator.Infraestructure.Models.Comment>()
            .ForMember(
                infraComment => infraComment.Id,
                opts => opts.MapFrom(domainComment => domainComment.Id)
            )
            .ForMember(
                infraComment => infraComment.PostId,
                opts => opts.MapFrom(domainComment => domainComment.PostId)
            )
            .ForMember(
                infraComment => infraComment.ParentCommentId,
                opts => opts.MapFrom(domainComment => domainComment.ParentCommentId)
            )
            .ForMember(
                infraComment => infraComment.AuthorId,
                opts => opts.MapFrom(domainComment => domainComment.Author.Id)
            )
            .ForMember(
                infraComment => infraComment.Content,
                opts => opts.MapFrom(domainComment => domainComment.Content)
            )
            .ForMember(
                infraComment => infraComment.Deleted,
                opts => opts.MapFrom(domainComment => domainComment.Deleted)
            );
    }
}