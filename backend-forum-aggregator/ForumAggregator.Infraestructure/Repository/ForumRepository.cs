...
public class ForumRepository : IForumRepository
{
    ...
    public Domain.ForumRegistry.Forum? Get(Guid forumId)
    {
        Models.Forum? forum = _dbContext.Forums
            .Where(forum => forum.Id == forumId && !forum.Deleted)
    ...
    {
        return _dbContext.Forums.OrderByDescending(x => x.CreatedAt).Select(x => _mapper.Map<Domain.ForumRegistry.Forum>(x)).ToList();
    }

    public ICollection<Domain.ForumRegistry.Forum> GetAllFromUser(Guid userId)
    {
        var posts = _dbContext.Forums.Where(x => x.OwnerId == userId && !x.Deleted).OrderByDescending(x => x.CreatedAt).ToList();
        return posts.Select(x => _mapper.Map<Domain.ForumRegistry.Forum>(x)).ToList();
    }
