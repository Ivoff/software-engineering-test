...
public class PostController: ControllerBase
{
    ...
    [HttpGet("post")]
    [AllowAnonymous]
    public IActionResult ReadAllPosts()
    {        
        return Ok(_mapper.Map<ICollection<PostAppServiceModel>, ICollection<ReadPostResponse>>(_postService.GetAllPosts()));
    }

    [HttpGet("post/recent/{count}")]
    [AllowAnonymous]
    public IActionResult ReadRecent(int count)
    {        
        if (count == 0)
            return BadRequest("Specify the amount of recent posts to take.");
        
        return Ok(_mapper.Map<ICollection<PostAppServiceModel>, ICollection<ReadPostResponse>>(_postService.GetRecentPosts(count)));
    }
}